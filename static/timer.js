    ...
    timer = setInterval(function () {
        if (!paused) {
            ...
            if (elapsedTime >= timerDuration) {
                audio.play();
                let id=document.querySelector('header').childElementCount+1;
                document.querySelector('header').innerHTML += `
                <div class="alert" id="beep${id}">
                    <p>Timer Finished</p>
                    <svg onclick="hide_alert(${id});audio.pause();audio.currentTime=0;" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                        viewBox="0 0 512 512">
                        <path
...
function resetTimer() {
    ...
    document.querySelector('#playing_controls').classList.toggle('hide');
    document.querySelector('.timer_clock').style.borderColor = 'var(--accent)';
    localStorage.setItem('timer_duration', 0);
    document.querySelector('#timer').style = `--timer-start: 0`;
}
...
if (localStorage.getItem('timer_duration')) {
    ...
    if (timeLeft > 0) {
        document.querySelector('#timer').style = `--timer-start: ${100 * ((currentTime() - first_epoch) / duration)}%`;
